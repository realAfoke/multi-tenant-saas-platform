	...
	return (
		<div className="max-w-4xl mx-auto space-y-8 text-white">
			...
			<div className="flex gap-3">
				<Card className={`bg-zinc-900 border-zinc-800 ${!addMembers ? 'flex-1' : ''}`}>
					< CardContent className="p-7 space-y-7">
						...
						<div className="grid md:grid-cols-2 gap-5">
							...
							<div className="rounded-xl border border-zinc-800 bg-zinc-950 p-5">
								...
								<div className="flex items-center gap-3" onClick={() => setAddMembers(prev => !prev)}>
									...
									<div>
										...
										<p className="text-xs text-zinc-500 mt-1">
											{projectMembers?.length >= 1 ? `${projectMembers?.length} user${projectMembers?.length > 1 ? '`s' : ''} added` : 'Add people to this project'}
										</p>
									</div>
								</div>
							...
							<div className="flex justify-end gap-3 pt-3">
								...
								<Button className="rounded-xl bg-blue-500 hover:bg-blue-600 px-6" onClick={() => {
									createProject.mutate({ wk: workspace?.id, data: { name: name, description: description, members: projectMembers } }, { onSuccess: (channel) => navigate(`../${channel?.name}`) })
								}}>
									Create Project
								</Button>
				...
				{addMembers &&
					<div className="p-3 border-l border-red-zinc flex-1">
						<div className="max-h-64 overflow-auto">
							{members?.map((member) => (
								<User key={member.user.id} member={member} projectMembers={projectMembers} addUserToProject={setProjectMembers} />
							))}
						</div>
					</div>
				}
