...
export default function SideBar(props) {
	...
	const { setToggle, handleWk, handleProject, selectedWorkspace, selectedProject, handleToggleWorkspace, toggleWorkspace } = props
	...
	const path = isProjectSelected ? `${selectedWorkspace?.id}/${selectedProject?.id}/add-new-task` : isWorkspaceSeleted ? `${selectedWorkspace?.id}/add-new-project` : 'create-new-workspace'
	...
	return (
		<div ref={ref} className={`overflow-y-auto scrollbar scrollbar-thin scrollbar-thumb-zinc-600 scrollbar-track-zinc-900 text-white bg-[#131011] absolute shadow-lg border-r border-[#f6f3f438] min-h-screen px-2 min-w-60 md:relative md:min-w-[20rem] z-9999 `}>
			<div className="flex justify-between items-center">
				<div className="text-2xl font-semibolf p-3 text-white shadow-md">Orbit</div>
				<div>
					<img src={closeMenu} className="w-5 h-5 md:w-5 md:h-5 cursor-pointer" onClick={() => setToggle(false)} />
				</div>
			</div>
			<div>
				...
				<Item className={`gap-2 py-2 ${!showOptions ? 'hover:bg-[#ffffff1a]' : ''} rounded-md px-2`}>
					...
					<ItemContent >
						...
						{showOptions && <ItemGroup className="gap-0">
							<Item className="py-2 my-0 hover:bg-[#ffffff1a] rounded-md px-2" onClick={() => navigate(path)}>
								<ItemContent>
									<ItemTitle className="capitalize text-sm w-full">
										{title}
									</ItemTitle>
									...
									<NestedList key={project?.id} list={project} selectedPrjId={selectedProject} setter={handleProject} />
									{
										...
										<ItemGroup className="gap-0 my-0 py-0 px-7">
											{project?.tasks?.map((taskId) => {
												...
												return (
													<NestedList key={task?.id} list={task} />
												)
											})}
										</ItemGroup>
									}
