...
export default function Plan(props) {
	...
	const { features = {} } = plan
	return (
		<div className="bg-[#333333] p-5 flex  md:p-10 flex-col gap-5 rounded-md hover:scale-103">
			<Button onClick={() => handleCheckout.mutate(plan?.id)} className={`${plan?.name === 'Business' ? 'bg-orange-500' : plan?.name === 'Pro' ? 'bg-purple-500' : 'bg-blue-500'} rounded-sm shadow-lg py-5`}>
				{`Get ${plan?.name}`}
			</Button>
